...
If (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]'Administrator') -Or $host.name -eq 'Windows Powershell ISE Host') {
    Write-Host "The Shared Mailbox Script will now open in PowerShell with Administrator Privilages, as you did not run the script as an Administrator or you ran the script in PowerShell ISE."
    Start-Sleep 1
    Write-Host "                                               3"
    Start-Sleep 1
    Write-Host "                                               2"
